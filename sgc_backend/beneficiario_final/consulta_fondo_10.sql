--SE TIENE QUE TENER EN CUENTA QUE UN PRODUCTO PUEDE TENER MAS DE UN TITULAR, ES POR ESTO QUE HAY QUE ARREGLAR EN ESTE QUERY LOS PORCENTAJES DE PARTICIPACION
WITH UBICACION AS(
    SELECT * FROM GE_TCIUD 
    INNER JOIN GE_TDEPTO ON DEPTO_DEPTO=CIUD_DEPTO
    INNER JOIN GE_TPAIS ON PAIS_PAIS=DEPTO_PAIS
)
--select NUMERO_DE_IDENTIF,count(*) from (
SELECT DISTINCT
'1'  AS BEPJTIT,
'8'  AS BEPJEBEN,
'N'  AS BEPJCON,
...
FROM (
    ...
    CLNAT_PNOMBRE,CLNAT_SNOMBRE,TO_CHAR(CLNAT_FECHA_NACIMIENTO,'YYYY-MM-DD') AS FEC_NAC,CIUD_NAC.PAIS_COD_DIAN PAIS_NACIMIENTO,PAIS_NACIO.PAIS_COD_DIAN PAIS_NACIONALIDAD,
    PAIS_RES.PAIS_COD_DIAN PAIS_RESIDENCIA,PAIS_RES.DEPTO_DEPTO DEPARTAMENTO_NOTIFICACION,
    PAIS_RES.CIUD_DANE MUNICIPIO_NOTIFICACION,PAIS_RES.DIREC_DIRECCION DIRECCION_NOTIFICACION,CODIGO_POSTAL,EMAIL_DIRECCION AS CORREO_NOTIFICACION,TO_CHAR(MAX_FECCRE,'YYYY-MM-DD') AS MAX_FECCRE,
    DIREC_BARRIO,
    PORCENTAJE_SALDO
    FROM  
        (
            SELECT  PTPL_FDEI,ROUND(SUM(NVL(ACDI_SALDO,0))*100/(SELECT SUM(ACDI_SALDO) FROM SF_TACDI
                WHERE ACDI_FECCIE=TO_DATE('2023-06-30','YYYY-MM-DD') AND ACDI_FOND='10'),5) PORCENTAJE_SALDO ,MAX(PTPL_FECCRE) MAX_FECCRE FROM SF_TPTPL  
            INNER JOIN SF_TPLAN  ON PLAN_PLAN=PTPL_PLAN 
            INNER JOIN SF_TFDEI ON FDEI_FDEI=PTPL_FDEI
            LEFT JOIN (SELECT * FROM SF_TACDI
                        WHERE ACDI_FECCIE=TO_DATE('2023-06-30','YYYY-MM-DD') AND ACDI_FOND='10') ON ACDI_PLAN=PLAN_PLAN
            WHERE PTPL_FOND='10' AND 
            TRUNC(PTPL_FECCRE)<=TRUNC(TO_DATE('2023-06-30','YYYY-MM-DD')) AND 
            (TRUNC(PLAN_FECCAN) > TRUNC(TO_DATE('2023-06-30', 'YYYY-MM-DD')) OR PLAN_FECCAN IS NULL) AND
            FDEI_TPID NOT IN ('A','F') and PTPL_TIPO='T'
            GROUP BY PTPL_FDEI
            ORDER BY 2 DESC
        ) C1 
          INNER JOIN SF_TPTPL C2 ON C1.PTPL_FDEI=C2.PTPL_FDEI AND C1.MAX_FECCRE=C2.PTPL_FECCRE 
          INNER JOIN SF_TFDEI ON C2.PTPL_FDEI=FDEI_FDEI
          INNER JOIN CL_TCLNTE  ON CLNTE_NROIDENT=C1.PTPL_FDEI and CLNTE_TPIDENTIF=DECODE(fdei_tpid,'C','CC','F','FF','A','NJ','E','CE','T','TI','R','RC','O','CC', 'P', 'PA')
          INNER JOIN CL_TCLNAT  ON CLNAT_NROIDENT=CLNTE_NROIDENT AND CLNAT_TPIDENTIF=CLNTE_TPIDENTIF
          ...
          LEFT JOIN CL_TCNLD ON CNAT_NROIDENTF=CLNTE_NROIDENT AND CNAT_TPIDENTIF=CLNAT_TPIDENTIF
          ...
          LEFT JOIN (
                        SELECT DISTINCT A.EMAIL_TPIDENTIF,A.EMAIL_NROIDENT,A.EMAIL_DIRECCION FROM CL_TEMAIL A INNER JOIN 
                            (
                                SELECT EMAIL_NROIDENT,MAX(EMAIL_EMAIL) MAX_EMA FROM CL_TEMAIL GROUP BY EMAIL_NROIDENT
                                
                            ) B ON A.EMAIL_EMAIL=B.MAX_EMA
                    ) EMA ON EMA.EMAIL_NROIDENT=CLNTE_NROIDENT          
)
WHERE (PAIS_EXPEDICION IS NOT NULL AND FEC_NAC IS NOT NULL AND PAIS_NACIMIENTO IS NOT NULL AND PAIS_NACIONALIDAD IS NOT NULL AND PAIS_RESIDENCIA IS NOT NULL AND DEPARTAMENTO_NOTIFICACION IS NOT NULL
AND MUNICIPIO_NOTIFICACION IS NOT NULL AND DIRECCION_NOTIFICACION IS NOT NULL AND CORREO_NOTIFICACION IS NOT NULL AND CODIGO_POSTAL IS NOT NULL AND MUNICIPIO_NOTIFICACION<>'-') 
--AND CLNTE_NROIDENT='1000284698'
--) GROUP BY NUMERO_DE_IDENTIF ORDER BY 2 DE
            ...
            select * from SF_TPtpl where  PTPL_FOND='10'