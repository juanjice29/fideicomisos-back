    ...
    FROM  
        (
            SELECT  PTPL_FDEI,ROUND(SUM(NVL(ACDI_SALDO,0))*100/(SELECT SUM(ACDI_SALDO) FROM SF_TACDI
                WHERE ACDI_FECCIE=TO_DATE('2023-06-30','YYYY-MM-DD') AND ACDI_FOND='12'),5) PORCENTAJE_SALDO ,MAX(PTPL_FECCRE) MAX_FECCRE FROM SF_TPTPL  
            INNER JOIN SF_TPLAN  ON PLAN_PLAN=PTPL_PLAN 
            INNER JOIN SF_TFDEI ON FDEI_FDEI=PTPL_FDEI
            LEFT JOIN (SELECT * FROM SF_TACDI
                        WHERE ACDI_FECCIE=TO_DATE('2023-06-30','YYYY-MM-DD') AND ACDI_FOND='12') ON ACDI_PLAN=PLAN_PLAN
            WHERE PTPL_FOND='12' AND 
